...
(* A function cannot change the variables it can refer to.  It can only
   temporarily shadow them with new variables that have the same names.  In this sense, variables are really constants and only behave like variables when dealing with recursion.  For this reason, variables are also called value bindings. An example of this: *)
...
(* Parameters of a type constructor are type variable, which are used in the data structures, polymorphism
BE ABLE TO TELL WHAT TYPES WILL BE FOR SOME 4; SOME 10; SOME "monkey";  ALSO BUNCHES*)
...
fun myfoldr f x nil = x
  | myfoldr f x (a::b) = f(a, myfoldr f x b);

myfoldr (op -) 0 [1, 2, 3, 4];